        ...
        if trading_calendar is None:
            ...
            raise ValueError(
                ...
                "sim_params.trading_calendar={}".format(
                    trading_calendar.name, self.sim_params.trading_calendar.name
                )
            )
...
""".strip().format(
    ...
    def _create_clock(self):
        ...
        """
        trading_o_and_c = self.trading_calendar.schedule.ix[self.sim_params.sessions]
    ...
    @api_method
    @preprocess(
        symbol_str=ensure_upper_case, country_code=optionally(ensure_upper_case)
    )
    def symbol(self, symbol_str, country_code=None):
        ...
        return self.asset_finder.lookup_symbol(
            symbol_str, as_of_date=_lookup_date, country_code=country_code
        )
    ...
    def order(
        ...
        return self.blotter.order(
            asset, amount, style, target_lots=target_lots, closing_rule=closing_rule
        )
    ...
    def order_target_percent(
        ...
        **kwargs
        ...
        style : ExecutionStyle
            The execution style for the order.
        *args, **kwargs : additional arguments to be passed to order
    ...
    @expect_types(
        pipeline=Pipeline, name=string_types, chunks=(int, Iterable, type(None))
    )
    ...
    def all_api_methods(cls):
        ...
        """
        return [
            fn for fn in itervalues(vars(cls)) if getattr(fn, "is_api_method", False)
        ]
